"use strict";
// Code generated by protoc-gen-ts_proto. DO NOT EDIT.
// versions:
//   protoc-gen-ts_proto  v2.7.7
//   protoc               unknown
// source: zitadel/group/v2/group_service.proto
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.GroupServiceDefinition = exports.DeleteGroupResponse = exports.DeleteGroupRequest = exports.UpdateGroupResponse = exports.UpdateGroupRequest = exports.GetGroupResponse = exports.GetGroupRequest = exports.ListGroupsResponse = exports.ListGroupsRequest = exports.CreateGroupResponse = exports.CreateGroupRequest = exports.protobufPackage = void 0;
/* eslint-disable */
const wire_1 = require("@bufbuild/protobuf/wire");
const long_1 = __importDefault(require("long"));
const timestamp_js_1 = require("../../../google/protobuf/timestamp.js");
const filter_js_1 = require("../../filter/v2/filter.js");
const group_js_1 = require("./group.js");
exports.protobufPackage = "zitadel.group.v2";
function createBaseCreateGroupRequest() {
    return { organizationId: "", name: "", description: "", id: undefined };
}
exports.CreateGroupRequest = {
    encode(message, writer = new wire_1.BinaryWriter()) {
        if (message.organizationId !== "") {
            writer.uint32(10).string(message.organizationId);
        }
        if (message.name !== "") {
            writer.uint32(18).string(message.name);
        }
        if (message.description !== "") {
            writer.uint32(26).string(message.description);
        }
        if (message.id !== undefined) {
            writer.uint32(34).string(message.id);
        }
        return writer;
    },
    decode(input, length) {
        const reader = input instanceof wire_1.BinaryReader ? input : new wire_1.BinaryReader(input);
        const end = length === undefined ? reader.len : reader.pos + length;
        const message = createBaseCreateGroupRequest();
        while (reader.pos < end) {
            const tag = reader.uint32();
            switch (tag >>> 3) {
                case 1: {
                    if (tag !== 10) {
                        break;
                    }
                    message.organizationId = reader.string();
                    continue;
                }
                case 2: {
                    if (tag !== 18) {
                        break;
                    }
                    message.name = reader.string();
                    continue;
                }
                case 3: {
                    if (tag !== 26) {
                        break;
                    }
                    message.description = reader.string();
                    continue;
                }
                case 4: {
                    if (tag !== 34) {
                        break;
                    }
                    message.id = reader.string();
                    continue;
                }
            }
            if ((tag & 7) === 4 || tag === 0) {
                break;
            }
            reader.skip(tag & 7);
        }
        return message;
    },
    fromJSON(object) {
        return {
            organizationId: isSet(object.organizationId) ? globalThis.String(object.organizationId) : "",
            name: isSet(object.name) ? globalThis.String(object.name) : "",
            description: isSet(object.description) ? globalThis.String(object.description) : "",
            id: isSet(object.id) ? globalThis.String(object.id) : undefined,
        };
    },
    toJSON(message) {
        const obj = {};
        if (message.organizationId !== "") {
            obj.organizationId = message.organizationId;
        }
        if (message.name !== "") {
            obj.name = message.name;
        }
        if (message.description !== "") {
            obj.description = message.description;
        }
        if (message.id !== undefined) {
            obj.id = message.id;
        }
        return obj;
    },
    create(base) {
        return exports.CreateGroupRequest.fromPartial(base ?? {});
    },
    fromPartial(object) {
        const message = createBaseCreateGroupRequest();
        message.organizationId = object.organizationId ?? "";
        message.name = object.name ?? "";
        message.description = object.description ?? "";
        message.id = object.id ?? undefined;
        return message;
    },
};
function createBaseCreateGroupResponse() {
    return { id: "", creationDate: undefined };
}
exports.CreateGroupResponse = {
    encode(message, writer = new wire_1.BinaryWriter()) {
        if (message.id !== "") {
            writer.uint32(10).string(message.id);
        }
        if (message.creationDate !== undefined) {
            timestamp_js_1.Timestamp.encode(toTimestamp(message.creationDate), writer.uint32(18).fork()).join();
        }
        return writer;
    },
    decode(input, length) {
        const reader = input instanceof wire_1.BinaryReader ? input : new wire_1.BinaryReader(input);
        const end = length === undefined ? reader.len : reader.pos + length;
        const message = createBaseCreateGroupResponse();
        while (reader.pos < end) {
            const tag = reader.uint32();
            switch (tag >>> 3) {
                case 1: {
                    if (tag !== 10) {
                        break;
                    }
                    message.id = reader.string();
                    continue;
                }
                case 2: {
                    if (tag !== 18) {
                        break;
                    }
                    message.creationDate = fromTimestamp(timestamp_js_1.Timestamp.decode(reader, reader.uint32()));
                    continue;
                }
            }
            if ((tag & 7) === 4 || tag === 0) {
                break;
            }
            reader.skip(tag & 7);
        }
        return message;
    },
    fromJSON(object) {
        return {
            id: isSet(object.id) ? globalThis.String(object.id) : "",
            creationDate: isSet(object.creationDate) ? fromJsonTimestamp(object.creationDate) : undefined,
        };
    },
    toJSON(message) {
        const obj = {};
        if (message.id !== "") {
            obj.id = message.id;
        }
        if (message.creationDate !== undefined) {
            obj.creationDate = message.creationDate.toISOString();
        }
        return obj;
    },
    create(base) {
        return exports.CreateGroupResponse.fromPartial(base ?? {});
    },
    fromPartial(object) {
        const message = createBaseCreateGroupResponse();
        message.id = object.id ?? "";
        message.creationDate = object.creationDate ?? undefined;
        return message;
    },
};
function createBaseListGroupsRequest() {
    return { filters: [], pagination: undefined, sortingColumn: undefined };
}
exports.ListGroupsRequest = {
    encode(message, writer = new wire_1.BinaryWriter()) {
        for (const v of message.filters) {
            group_js_1.GroupsSearchFilter.encode(v, writer.uint32(10).fork()).join();
        }
        if (message.pagination !== undefined) {
            filter_js_1.PaginationRequest.encode(message.pagination, writer.uint32(18).fork()).join();
        }
        if (message.sortingColumn !== undefined) {
            writer.uint32(24).int32(message.sortingColumn);
        }
        return writer;
    },
    decode(input, length) {
        const reader = input instanceof wire_1.BinaryReader ? input : new wire_1.BinaryReader(input);
        const end = length === undefined ? reader.len : reader.pos + length;
        const message = createBaseListGroupsRequest();
        while (reader.pos < end) {
            const tag = reader.uint32();
            switch (tag >>> 3) {
                case 1: {
                    if (tag !== 10) {
                        break;
                    }
                    message.filters.push(group_js_1.GroupsSearchFilter.decode(reader, reader.uint32()));
                    continue;
                }
                case 2: {
                    if (tag !== 18) {
                        break;
                    }
                    message.pagination = filter_js_1.PaginationRequest.decode(reader, reader.uint32());
                    continue;
                }
                case 3: {
                    if (tag !== 24) {
                        break;
                    }
                    message.sortingColumn = reader.int32();
                    continue;
                }
            }
            if ((tag & 7) === 4 || tag === 0) {
                break;
            }
            reader.skip(tag & 7);
        }
        return message;
    },
    fromJSON(object) {
        return {
            filters: globalThis.Array.isArray(object?.filters)
                ? object.filters.map((e) => group_js_1.GroupsSearchFilter.fromJSON(e))
                : [],
            pagination: isSet(object.pagination) ? filter_js_1.PaginationRequest.fromJSON(object.pagination) : undefined,
            sortingColumn: isSet(object.sortingColumn) ? (0, group_js_1.fieldNameFromJSON)(object.sortingColumn) : undefined,
        };
    },
    toJSON(message) {
        const obj = {};
        if (message.filters?.length) {
            obj.filters = message.filters.map((e) => group_js_1.GroupsSearchFilter.toJSON(e));
        }
        if (message.pagination !== undefined) {
            obj.pagination = filter_js_1.PaginationRequest.toJSON(message.pagination);
        }
        if (message.sortingColumn !== undefined) {
            obj.sortingColumn = (0, group_js_1.fieldNameToJSON)(message.sortingColumn);
        }
        return obj;
    },
    create(base) {
        return exports.ListGroupsRequest.fromPartial(base ?? {});
    },
    fromPartial(object) {
        const message = createBaseListGroupsRequest();
        message.filters = object.filters?.map((e) => group_js_1.GroupsSearchFilter.fromPartial(e)) || [];
        message.pagination = (object.pagination !== undefined && object.pagination !== null)
            ? filter_js_1.PaginationRequest.fromPartial(object.pagination)
            : undefined;
        message.sortingColumn = object.sortingColumn ?? undefined;
        return message;
    },
};
function createBaseListGroupsResponse() {
    return { groups: [], pagination: undefined };
}
exports.ListGroupsResponse = {
    encode(message, writer = new wire_1.BinaryWriter()) {
        for (const v of message.groups) {
            group_js_1.Group.encode(v, writer.uint32(10).fork()).join();
        }
        if (message.pagination !== undefined) {
            filter_js_1.PaginationResponse.encode(message.pagination, writer.uint32(18).fork()).join();
        }
        return writer;
    },
    decode(input, length) {
        const reader = input instanceof wire_1.BinaryReader ? input : new wire_1.BinaryReader(input);
        const end = length === undefined ? reader.len : reader.pos + length;
        const message = createBaseListGroupsResponse();
        while (reader.pos < end) {
            const tag = reader.uint32();
            switch (tag >>> 3) {
                case 1: {
                    if (tag !== 10) {
                        break;
                    }
                    message.groups.push(group_js_1.Group.decode(reader, reader.uint32()));
                    continue;
                }
                case 2: {
                    if (tag !== 18) {
                        break;
                    }
                    message.pagination = filter_js_1.PaginationResponse.decode(reader, reader.uint32());
                    continue;
                }
            }
            if ((tag & 7) === 4 || tag === 0) {
                break;
            }
            reader.skip(tag & 7);
        }
        return message;
    },
    fromJSON(object) {
        return {
            groups: globalThis.Array.isArray(object?.groups) ? object.groups.map((e) => group_js_1.Group.fromJSON(e)) : [],
            pagination: isSet(object.pagination) ? filter_js_1.PaginationResponse.fromJSON(object.pagination) : undefined,
        };
    },
    toJSON(message) {
        const obj = {};
        if (message.groups?.length) {
            obj.groups = message.groups.map((e) => group_js_1.Group.toJSON(e));
        }
        if (message.pagination !== undefined) {
            obj.pagination = filter_js_1.PaginationResponse.toJSON(message.pagination);
        }
        return obj;
    },
    create(base) {
        return exports.ListGroupsResponse.fromPartial(base ?? {});
    },
    fromPartial(object) {
        const message = createBaseListGroupsResponse();
        message.groups = object.groups?.map((e) => group_js_1.Group.fromPartial(e)) || [];
        message.pagination = (object.pagination !== undefined && object.pagination !== null)
            ? filter_js_1.PaginationResponse.fromPartial(object.pagination)
            : undefined;
        return message;
    },
};
function createBaseGetGroupRequest() {
    return { id: "" };
}
exports.GetGroupRequest = {
    encode(message, writer = new wire_1.BinaryWriter()) {
        if (message.id !== "") {
            writer.uint32(10).string(message.id);
        }
        return writer;
    },
    decode(input, length) {
        const reader = input instanceof wire_1.BinaryReader ? input : new wire_1.BinaryReader(input);
        const end = length === undefined ? reader.len : reader.pos + length;
        const message = createBaseGetGroupRequest();
        while (reader.pos < end) {
            const tag = reader.uint32();
            switch (tag >>> 3) {
                case 1: {
                    if (tag !== 10) {
                        break;
                    }
                    message.id = reader.string();
                    continue;
                }
            }
            if ((tag & 7) === 4 || tag === 0) {
                break;
            }
            reader.skip(tag & 7);
        }
        return message;
    },
    fromJSON(object) {
        return { id: isSet(object.id) ? globalThis.String(object.id) : "" };
    },
    toJSON(message) {
        const obj = {};
        if (message.id !== "") {
            obj.id = message.id;
        }
        return obj;
    },
    create(base) {
        return exports.GetGroupRequest.fromPartial(base ?? {});
    },
    fromPartial(object) {
        const message = createBaseGetGroupRequest();
        message.id = object.id ?? "";
        return message;
    },
};
function createBaseGetGroupResponse() {
    return { group: undefined };
}
exports.GetGroupResponse = {
    encode(message, writer = new wire_1.BinaryWriter()) {
        if (message.group !== undefined) {
            group_js_1.Group.encode(message.group, writer.uint32(10).fork()).join();
        }
        return writer;
    },
    decode(input, length) {
        const reader = input instanceof wire_1.BinaryReader ? input : new wire_1.BinaryReader(input);
        const end = length === undefined ? reader.len : reader.pos + length;
        const message = createBaseGetGroupResponse();
        while (reader.pos < end) {
            const tag = reader.uint32();
            switch (tag >>> 3) {
                case 1: {
                    if (tag !== 10) {
                        break;
                    }
                    message.group = group_js_1.Group.decode(reader, reader.uint32());
                    continue;
                }
            }
            if ((tag & 7) === 4 || tag === 0) {
                break;
            }
            reader.skip(tag & 7);
        }
        return message;
    },
    fromJSON(object) {
        return { group: isSet(object.group) ? group_js_1.Group.fromJSON(object.group) : undefined };
    },
    toJSON(message) {
        const obj = {};
        if (message.group !== undefined) {
            obj.group = group_js_1.Group.toJSON(message.group);
        }
        return obj;
    },
    create(base) {
        return exports.GetGroupResponse.fromPartial(base ?? {});
    },
    fromPartial(object) {
        const message = createBaseGetGroupResponse();
        message.group = (object.group !== undefined && object.group !== null) ? group_js_1.Group.fromPartial(object.group) : undefined;
        return message;
    },
};
function createBaseUpdateGroupRequest() {
    return { id: "", name: undefined, description: undefined };
}
exports.UpdateGroupRequest = {
    encode(message, writer = new wire_1.BinaryWriter()) {
        if (message.id !== "") {
            writer.uint32(10).string(message.id);
        }
        if (message.name !== undefined) {
            writer.uint32(18).string(message.name);
        }
        if (message.description !== undefined) {
            writer.uint32(26).string(message.description);
        }
        return writer;
    },
    decode(input, length) {
        const reader = input instanceof wire_1.BinaryReader ? input : new wire_1.BinaryReader(input);
        const end = length === undefined ? reader.len : reader.pos + length;
        const message = createBaseUpdateGroupRequest();
        while (reader.pos < end) {
            const tag = reader.uint32();
            switch (tag >>> 3) {
                case 1: {
                    if (tag !== 10) {
                        break;
                    }
                    message.id = reader.string();
                    continue;
                }
                case 2: {
                    if (tag !== 18) {
                        break;
                    }
                    message.name = reader.string();
                    continue;
                }
                case 3: {
                    if (tag !== 26) {
                        break;
                    }
                    message.description = reader.string();
                    continue;
                }
            }
            if ((tag & 7) === 4 || tag === 0) {
                break;
            }
            reader.skip(tag & 7);
        }
        return message;
    },
    fromJSON(object) {
        return {
            id: isSet(object.id) ? globalThis.String(object.id) : "",
            name: isSet(object.name) ? globalThis.String(object.name) : undefined,
            description: isSet(object.description) ? globalThis.String(object.description) : undefined,
        };
    },
    toJSON(message) {
        const obj = {};
        if (message.id !== "") {
            obj.id = message.id;
        }
        if (message.name !== undefined) {
            obj.name = message.name;
        }
        if (message.description !== undefined) {
            obj.description = message.description;
        }
        return obj;
    },
    create(base) {
        return exports.UpdateGroupRequest.fromPartial(base ?? {});
    },
    fromPartial(object) {
        const message = createBaseUpdateGroupRequest();
        message.id = object.id ?? "";
        message.name = object.name ?? undefined;
        message.description = object.description ?? undefined;
        return message;
    },
};
function createBaseUpdateGroupResponse() {
    return { changeDate: undefined };
}
exports.UpdateGroupResponse = {
    encode(message, writer = new wire_1.BinaryWriter()) {
        if (message.changeDate !== undefined) {
            timestamp_js_1.Timestamp.encode(toTimestamp(message.changeDate), writer.uint32(10).fork()).join();
        }
        return writer;
    },
    decode(input, length) {
        const reader = input instanceof wire_1.BinaryReader ? input : new wire_1.BinaryReader(input);
        const end = length === undefined ? reader.len : reader.pos + length;
        const message = createBaseUpdateGroupResponse();
        while (reader.pos < end) {
            const tag = reader.uint32();
            switch (tag >>> 3) {
                case 1: {
                    if (tag !== 10) {
                        break;
                    }
                    message.changeDate = fromTimestamp(timestamp_js_1.Timestamp.decode(reader, reader.uint32()));
                    continue;
                }
            }
            if ((tag & 7) === 4 || tag === 0) {
                break;
            }
            reader.skip(tag & 7);
        }
        return message;
    },
    fromJSON(object) {
        return { changeDate: isSet(object.changeDate) ? fromJsonTimestamp(object.changeDate) : undefined };
    },
    toJSON(message) {
        const obj = {};
        if (message.changeDate !== undefined) {
            obj.changeDate = message.changeDate.toISOString();
        }
        return obj;
    },
    create(base) {
        return exports.UpdateGroupResponse.fromPartial(base ?? {});
    },
    fromPartial(object) {
        const message = createBaseUpdateGroupResponse();
        message.changeDate = object.changeDate ?? undefined;
        return message;
    },
};
function createBaseDeleteGroupRequest() {
    return { id: "" };
}
exports.DeleteGroupRequest = {
    encode(message, writer = new wire_1.BinaryWriter()) {
        if (message.id !== "") {
            writer.uint32(10).string(message.id);
        }
        return writer;
    },
    decode(input, length) {
        const reader = input instanceof wire_1.BinaryReader ? input : new wire_1.BinaryReader(input);
        const end = length === undefined ? reader.len : reader.pos + length;
        const message = createBaseDeleteGroupRequest();
        while (reader.pos < end) {
            const tag = reader.uint32();
            switch (tag >>> 3) {
                case 1: {
                    if (tag !== 10) {
                        break;
                    }
                    message.id = reader.string();
                    continue;
                }
            }
            if ((tag & 7) === 4 || tag === 0) {
                break;
            }
            reader.skip(tag & 7);
        }
        return message;
    },
    fromJSON(object) {
        return { id: isSet(object.id) ? globalThis.String(object.id) : "" };
    },
    toJSON(message) {
        const obj = {};
        if (message.id !== "") {
            obj.id = message.id;
        }
        return obj;
    },
    create(base) {
        return exports.DeleteGroupRequest.fromPartial(base ?? {});
    },
    fromPartial(object) {
        const message = createBaseDeleteGroupRequest();
        message.id = object.id ?? "";
        return message;
    },
};
function createBaseDeleteGroupResponse() {
    return { deletionDate: undefined };
}
exports.DeleteGroupResponse = {
    encode(message, writer = new wire_1.BinaryWriter()) {
        if (message.deletionDate !== undefined) {
            timestamp_js_1.Timestamp.encode(toTimestamp(message.deletionDate), writer.uint32(10).fork()).join();
        }
        return writer;
    },
    decode(input, length) {
        const reader = input instanceof wire_1.BinaryReader ? input : new wire_1.BinaryReader(input);
        const end = length === undefined ? reader.len : reader.pos + length;
        const message = createBaseDeleteGroupResponse();
        while (reader.pos < end) {
            const tag = reader.uint32();
            switch (tag >>> 3) {
                case 1: {
                    if (tag !== 10) {
                        break;
                    }
                    message.deletionDate = fromTimestamp(timestamp_js_1.Timestamp.decode(reader, reader.uint32()));
                    continue;
                }
            }
            if ((tag & 7) === 4 || tag === 0) {
                break;
            }
            reader.skip(tag & 7);
        }
        return message;
    },
    fromJSON(object) {
        return { deletionDate: isSet(object.deletionDate) ? fromJsonTimestamp(object.deletionDate) : undefined };
    },
    toJSON(message) {
        const obj = {};
        if (message.deletionDate !== undefined) {
            obj.deletionDate = message.deletionDate.toISOString();
        }
        return obj;
    },
    create(base) {
        return exports.DeleteGroupResponse.fromPartial(base ?? {});
    },
    fromPartial(object) {
        const message = createBaseDeleteGroupResponse();
        message.deletionDate = object.deletionDate ?? undefined;
        return message;
    },
};
exports.GroupServiceDefinition = {
    name: "GroupService",
    fullName: "zitadel.group.v2.GroupService",
    methods: {
        /**
         * CreateGroup
         *
         * CreateGroup creates a new user group in an organization.
         *
         * Required permissions: // TODO
         *   - "iam.member.write" for instance administrators
         *   - "org.member.write" for organization administrators
         */
        createGroup: {
            name: "CreateGroup",
            requestType: exports.CreateGroupRequest,
            requestStream: false,
            responseType: exports.CreateGroupResponse,
            responseStream: false,
            options: {
                _unknownFields: {
                    8338: [
                        Buffer.from([
                            196,
                            1,
                            74,
                            48,
                            10,
                            3,
                            50,
                            48,
                            49,
                            18,
                            41,
                            10,
                            39,
                            84,
                            104,
                            101,
                            32,
                            117,
                            115,
                            101,
                            114,
                            32,
                            103,
                            114,
                            111,
                            117,
                            112,
                            32,
                            119,
                            97,
                            115,
                            32,
                            99,
                            114,
                            101,
                            97,
                            116,
                            101,
                            100,
                            32,
                            115,
                            117,
                            99,
                            99,
                            101,
                            115,
                            115,
                            102,
                            117,
                            108,
                            108,
                            121,
                            74,
                            74,
                            10,
                            3,
                            52,
                            48,
                            48,
                            18,
                            67,
                            10,
                            36,
                            73,
                            110,
                            118,
                            97,
                            108,
                            105,
                            100,
                            32,
                            114,
                            101,
                            113,
                            117,
                            101,
                            115,
                            116,
                            32,
                            116,
                            111,
                            32,
                            99,
                            114,
                            101,
                            97,
                            116,
                            101,
                            32,
                            117,
                            115,
                            101,
                            114,
                            32,
                            103,
                            114,
                            111,
                            117,
                            112,
                            18,
                            27,
                            10,
                            25,
                            26,
                            23,
                            35,
                            47,
                            100,
                            101,
                            102,
                            105,
                            110,
                            105,
                            116,
                            105,
                            111,
                            110,
                            115,
                            47,
                            114,
                            112,
                            99,
                            83,
                            116,
                            97,
                            116,
                            117,
                            115,
                            74,
                            68,
                            10,
                            3,
                            52,
                            48,
                            57,
                            18,
                            61,
                            10,
                            30,
                            84,
                            104,
                            101,
                            32,
                            117,
                            115,
                            101,
                            114,
                            32,
                            103,
                            114,
                            111,
                            117,
                            112,
                            32,
                            97,
                            108,
                            114,
                            101,
                            97,
                            100,
                            121,
                            32,
                            101,
                            120,
                            105,
                            115,
                            116,
                            115,
                            46,
                            18,
                            27,
                            10,
                            25,
                            26,
                            23,
                            35,
                            47,
                            100,
                            101,
                            102,
                            105,
                            110,
                            105,
                            116,
                            105,
                            111,
                            110,
                            115,
                            47,
                            114,
                            112,
                            99,
                            83,
                            116,
                            97,
                            116,
                            117,
                            115,
                        ]),
                    ],
                    400010: [
                        Buffer.from([
                            22,
                            10,
                            15,
                            10,
                            13,
                            97,
                            117,
                            116,
                            104,
                            101,
                            110,
                            116,
                            105,
                            99,
                            97,
                            116,
                            101,
                            100,
                            18,
                            3,
                            8,
                            201,
                            1,
                        ]),
                    ],
                },
            },
        },
        /**
         * GetGroup
         *
         * Retrieves a group based on its ID.
         *
         * Required permission:
         * - TODO
         */
        getGroup: {
            name: "GetGroup",
            requestType: exports.GetGroupRequest,
            requestStream: false,
            responseType: exports.GetGroupResponse,
            responseStream: false,
            options: {
                _unknownFields: {
                    8338: [
                        Buffer.from([
                            114,
                            74,
                            37,
                            10,
                            3,
                            50,
                            48,
                            48,
                            18,
                            30,
                            10,
                            28,
                            71,
                            114,
                            111,
                            117,
                            112,
                            32,
                            114,
                            101,
                            116,
                            114,
                            105,
                            101,
                            118,
                            101,
                            100,
                            32,
                            115,
                            117,
                            99,
                            99,
                            101,
                            115,
                            115,
                            102,
                            117,
                            108,
                            108,
                            121,
                            74,
                            73,
                            10,
                            3,
                            52,
                            48,
                            52,
                            18,
                            66,
                            10,
                            35,
                            84,
                            104,
                            101,
                            32,
                            114,
                            101,
                            113,
                            117,
                            101,
                            115,
                            116,
                            101,
                            100,
                            32,
                            103,
                            114,
                            111,
                            117,
                            112,
                            32,
                            100,
                            111,
                            101,
                            115,
                            32,
                            110,
                            111,
                            116,
                            32,
                            101,
                            120,
                            105,
                            115,
                            116,
                            46,
                            18,
                            27,
                            10,
                            25,
                            26,
                            23,
                            35,
                            47,
                            100,
                            101,
                            102,
                            105,
                            110,
                            105,
                            116,
                            105,
                            111,
                            110,
                            115,
                            47,
                            114,
                            112,
                            99,
                            83,
                            116,
                            97,
                            116,
                            117,
                            115,
                        ]),
                    ],
                    400010: [Buffer.from([17, 10, 15, 10, 13, 97, 117, 116, 104, 101, 110, 116, 105, 99, 97, 116, 101, 100])],
                },
            },
        },
        /**
         * ListGroups
         *
         * ListGroups returns all groups matching the request and necessary permissions from an organization.
         *
         * Required permissions: // TODO
         *   - "iam.member.read" for instance administrators
         *   - "org.member.read" for organization administrators
         */
        listGroups: {
            name: "ListGroups",
            requestType: exports.ListGroupsRequest,
            requestStream: false,
            responseType: exports.ListGroupsResponse,
            responseStream: false,
            options: {
                _unknownFields: {
                    8338: [
                        Buffer.from([
                            114,
                            74,
                            48,
                            10,
                            3,
                            50,
                            48,
                            48,
                            18,
                            41,
                            10,
                            39,
                            65,
                            32,
                            108,
                            105,
                            115,
                            116,
                            32,
                            111,
                            102,
                            32,
                            97,
                            108,
                            108,
                            32,
                            103,
                            114,
                            111,
                            117,
                            112,
                            115,
                            32,
                            109,
                            97,
                            116,
                            99,
                            104,
                            105,
                            110,
                            103,
                            32,
                            116,
                            104,
                            101,
                            32,
                            113,
                            117,
                            101,
                            114,
                            121,
                            74,
                            62,
                            10,
                            3,
                            52,
                            48,
                            48,
                            18,
                            55,
                            10,
                            24,
                            105,
                            110,
                            118,
                            97,
                            108,
                            105,
                            100,
                            32,
                            108,
                            105,
                            115,
                            116,
                            32,
                            103,
                            114,
                            111,
                            117,
                            112,
                            32,
                            113,
                            117,
                            101,
                            114,
                            121,
                            18,
                            27,
                            10,
                            25,
                            26,
                            23,
                            35,
                            47,
                            100,
                            101,
                            102,
                            105,
                            110,
                            105,
                            116,
                            105,
                            111,
                            110,
                            115,
                            47,
                            114,
                            112,
                            99,
                            83,
                            116,
                            97,
                            116,
                            117,
                            115,
                        ]),
                    ],
                    400010: [Buffer.from([17, 10, 15, 10, 13, 97, 117, 116, 104, 101, 110, 116, 105, 99, 97, 116, 101, 100])],
                },
            },
        },
        /**
         * UpdateGroup
         *
         * UpdateGroup updates the user group.
         *
         * In case the group there aren't any changes, the request will return a successful response as
         * the desired state is already achieved.
         * You can check the change date in the response to verify if the group was updated by the request.
         *
         * Required permissions: // TODO
         *   - "iam.member.write" for instance administrators
         *   - "org.member.write" for organization administrators
         */
        updateGroup: {
            name: "UpdateGroup",
            requestType: exports.UpdateGroupRequest,
            requestStream: false,
            responseType: exports.UpdateGroupResponse,
            responseStream: false,
            options: {
                _unknownFields: {
                    8338: [
                        Buffer.from([
                            138,
                            1,
                            74,
                            48,
                            10,
                            3,
                            50,
                            48,
                            48,
                            18,
                            41,
                            10,
                            39,
                            84,
                            104,
                            101,
                            32,
                            117,
                            115,
                            101,
                            114,
                            32,
                            103,
                            114,
                            111,
                            117,
                            112,
                            32,
                            119,
                            97,
                            115,
                            32,
                            117,
                            112,
                            100,
                            97,
                            116,
                            101,
                            100,
                            32,
                            115,
                            117,
                            99,
                            99,
                            101,
                            115,
                            115,
                            102,
                            117,
                            108,
                            108,
                            121,
                            74,
                            86,
                            10,
                            3,
                            52,
                            48,
                            52,
                            18,
                            79,
                            10,
                            48,
                            84,
                            104,
                            101,
                            32,
                            117,
                            115,
                            101,
                            114,
                            32,
                            103,
                            114,
                            111,
                            117,
                            112,
                            32,
                            111,
                            114,
                            32,
                            111,
                            110,
                            101,
                            32,
                            111,
                            102,
                            32,
                            116,
                            104,
                            101,
                            32,
                            114,
                            111,
                            108,
                            101,
                            115,
                            32,
                            100,
                            111,
                            32,
                            110,
                            111,
                            116,
                            32,
                            101,
                            120,
                            105,
                            115,
                            116,
                            46,
                            18,
                            27,
                            10,
                            25,
                            26,
                            23,
                            35,
                            47,
                            100,
                            101,
                            102,
                            105,
                            110,
                            105,
                            116,
                            105,
                            111,
                            110,
                            115,
                            47,
                            114,
                            112,
                            99,
                            83,
                            116,
                            97,
                            116,
                            117,
                            115,
                        ]),
                    ],
                    400010: [Buffer.from([17, 10, 15, 10, 13, 97, 117, 116, 104, 101, 110, 116, 105, 99, 97, 116, 101, 100])],
                },
            },
        },
        /**
         * DeleteGroup
         *
         * DeleteGroup deletes the group.
         *
         * In case the group is not found, the request will return a successful response as
         * the desired state is already achieved.
         * You can check the deletion date in the response to verify if the group was deleted by the request.
         *
         * Required permissions: // TODO
         *   - "iam.member.delete" for instance administrators
         *   - "org.member.delete" for organization administrators
         */
        deleteGroup: {
            name: "DeleteGroup",
            requestType: exports.DeleteGroupRequest,
            requestStream: false,
            responseType: exports.DeleteGroupResponse,
            responseStream: false,
            options: {
                _unknownFields: {
                    8338: [
                        Buffer.from([
                            116,
                            74,
                            49,
                            10,
                            3,
                            50,
                            48,
                            48,
                            18,
                            42,
                            10,
                            40,
                            84,
                            104,
                            101,
                            32,
                            117,
                            115,
                            101,
                            114,
                            32,
                            103,
                            114,
                            111,
                            117,
                            112,
                            32,
                            119,
                            97,
                            115,
                            32,
                            100,
                            101,
                            108,
                            101,
                            116,
                            101,
                            100,
                            32,
                            115,
                            117,
                            99,
                            99,
                            101,
                            115,
                            115,
                            102,
                            117,
                            108,
                            108,
                            121,
                            46,
                            74,
                            63,
                            10,
                            3,
                            52,
                            48,
                            52,
                            18,
                            56,
                            10,
                            25,
                            84,
                            104,
                            101,
                            32,
                            117,
                            115,
                            101,
                            114,
                            32,
                            103,
                            114,
                            111,
                            117,
                            112,
                            32,
                            110,
                            111,
                            116,
                            32,
                            102,
                            111,
                            117,
                            110,
                            100,
                            46,
                            18,
                            27,
                            10,
                            25,
                            26,
                            23,
                            35,
                            47,
                            100,
                            101,
                            102,
                            105,
                            110,
                            105,
                            116,
                            105,
                            111,
                            110,
                            115,
                            47,
                            114,
                            112,
                            99,
                            83,
                            116,
                            97,
                            116,
                            117,
                            115,
                        ]),
                    ],
                    400010: [Buffer.from([17, 10, 15, 10, 13, 97, 117, 116, 104, 101, 110, 116, 105, 99, 97, 116, 101, 100])],
                },
            },
        },
    },
};
function toTimestamp(date) {
    const seconds = numberToLong(Math.trunc(date.getTime() / 1_000));
    const nanos = (date.getTime() % 1_000) * 1_000_000;
    return { seconds, nanos };
}
function fromTimestamp(t) {
    let millis = (t.seconds.toNumber() || 0) * 1_000;
    millis += (t.nanos || 0) / 1_000_000;
    return new globalThis.Date(millis);
}
function fromJsonTimestamp(o) {
    if (o instanceof globalThis.Date) {
        return o;
    }
    else if (typeof o === "string") {
        return new globalThis.Date(o);
    }
    else {
        return fromTimestamp(timestamp_js_1.Timestamp.fromJSON(o));
    }
}
function numberToLong(number) {
    return long_1.default.fromNumber(number);
}
function isSet(value) {
    return value !== null && value !== undefined;
}
