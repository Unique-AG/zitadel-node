 ...
 */
export function createOidcClient(apiEndpoint: string, grpcHeadersOverwrite?: Partial<ChannelOptions>, ...interceptors: ClientMiddleware[]): OIDCServiceClient {
  const channel = createChannel(apiEndpoint, undefined, grpcHeadersOverwrite);
  let factory = createClientFactory();
  for (const interceptor of interceptors) {
    factory = factory.use(interceptor);
...
export function createOrganizationClient(
  apiEndpoint: string,
  grpcHeadersOverwrite?: Partial<ChannelOptions>,
